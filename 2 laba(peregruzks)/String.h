...
public:
	//Operations
	explicit String(const char* s = "");						//Конструктор с параметром строки.
	...
	char* operator () (int a, int b);							//Выделение подстроки.
	String operator + (const String& s);						//Перегрузка + с объектом.
	...
	friend String operator + (const char* s, String& Str);		//Перегрузка + (строка + объект)
	friend String operator + (char c, String& Str);				//Перегрузка + (символ + объект)
	//Comparing
	bool operator != (const String& s);							//Пеоегрузка != с объектом.
	...
	friend bool operator == (const char* s, String& Str);		//Глобальная перегрузка ==.
	friend bool operator != (const char* s, String& Str);		//Глобальная перегрузка !=.
	friend bool operator > (const char* s, String& Str);		//Глобальная перегрузка >.
	friend bool operator < (const char* s, String& Str);		//Глобальная перегрузка <.
};